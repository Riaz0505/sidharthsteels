    ...
    <div className="flex flex-col">
      ...
      <section className="relative h-[95vh] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-steel-950 via-steel-950/40 to-transparent" />
        </div>
        <div className="container-custom relative z-10">
          <motion.div 
            ...
            className="max-w-4xl"
          >
            <h1 className="font-display text-4xl sm:text-5xl md:text-8xl font-extrabold text-white leading-[1.1] mb-6 md:mb-8 tracking-[-0.02em] whitespace-pre-line">
              {settings.heroTitle.includes('India') ? (
                <>The Backbone of <br /><span className="text-steel-400">Industrial India</span></>
              ) : settings.heroTitle}
            </h1>
            <p className="text-sm sm:text-base md:text-2xl text-steel-200 mb-8 md:mb-12 font-medium max-w-2xl leading-relaxed">
              {settings.heroTagline}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-5">
              <Link to="/services" className="px-8 md:px-10 py-4 md:py-5 bg-primary text-white font-bold rounded-2xl hover:scale-105 transition-all flex items-center justify-center gap-3 shadow-2xl">
                Explore Inventory <ArrowRight size={20} />
              </Link>
              <Link to="/contact" className="px-8 md:px-10 py-4 md:py-5 border-2 border-white/20 text-white font-bold rounded-2xl hover:bg-white/10 transition-all backdrop-blur-sm flex items-center justify-center">
                Material Request
              </Link>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <motion.div 
                ...
                className="p-6 md:p-10 bg-white/95 backdrop-blur-xl border border-steel-100 rounded-[24px] md:rounded-[32px] shadow-2xl text-center group hover:-translate-y-2 transition-all duration-500"
              >
                <div className="w-12 h-12 md:w-16 md:h-16 bg-steel-950 text-white rounded-xl md:rounded-2xl flex items-center justify-center mx-auto mb-4 md:mb-6 group-hover:rotate-12 transition-transform">
                  <stat.icon size={22} />
                </div>
                <h3 className="text-2xl md:text-4xl font-bold text-steel-950 mb-1 tracking-tighter">{stat.value}</h3>
                <p className="text-steel-400 font-bold uppercase text-[8px] md:text-[10px] tracking-widest">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center">
            ...
            <div className="relative">
              <div className="aspect-square bg-steel-950 rounded-[64px] overflow-hidden shadow-2xl">
                <img 
                  src={p1} 
              ...
              {/* Floating badge */}
              <div className="absolute -bottom-10 -left-10 bg-white p-8 rounded-[40px] shadow-2xl border border-steel-100 max-w-[240px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {SERVICES.slice(0, 3).map((service, i) => (
              <motion.div 
                ...
                transition={{ delay: i * 0.1 }}
                className="group relative bg-steel-900/40 backdrop-blur-sm rounded-[48px] overflow-hidden border border-white/5 hover:border-white/20 transition-all duration-700 h-[600px]"
              >
                <div className="absolute inset-0 z-0">
                  <img 
                    src={service.image} 
                ...
                <div className="absolute bottom-0 left-0 right-0 p-10 z-10">
                   <span className="text-steel-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-4 block">Certified Product</span>
                   <h3 className="text-3xl font-bold mb-6 leading-tight">{service.title}</h3>
                   <p className="text-steel-300 text-sm mb-8 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 overflow-hidden line-clamp-3">
                     {service.description}
                   </p>
                   <Link to="/services" className="w-14 h-14 bg-white text-steel-950 rounded-full flex items-center justify-center group-hover:w-full transition-all duration-500 font-bold overflow-hidden">
                      <span className="hidden group-hover:block whitespace-nowrap px-6">View Specifications</span>
            ...
            {INDUSTRIES_DETAILED.map((industry, i) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-white p-12 rounded-[48px] border border-steel-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cursor-default group"
                >
                  <div className="w-16 h-16 rounded-2xl bg-steel-950 text-white flex items-center justify-center mb-8 group-hover:scale-110 transition-transform">
                    <Icon size={28} />
                  </div>
                  <h3 className="text-2xl font-bold text-steel-950 mb-4 tracking-tight">{industry.name}</h3>
                  <p className="text-steel-500 font-medium leading-relaxed">{industry.description}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section className="bg-steel-50 py-20 md:py-32 overflow-hidden px-4">
        <div className="container-custom">
           <div className="bg-steel-950 rounded-[32px] md:rounded-[64px] p-8 md:p-24 relative">
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center text-white">
                 <div>
                    <h2 className="text-3xl md:text-5xl font-bold mb-8 md:mb-12 leading-tight">Supply Chain <br /><span className="text-steel-500">Excellence</span></h2>
                    <div className="space-y-8 md:space-y-12">
                       {[
                         ...
                         <div key={item.step} className="flex gap-4 md:gap-8 group">
                            <span className="text-2xl md:text-4xl font-bold text-steel-800 transition-colors group-hover:text-white">{item.step}</span>
                            <div>
                               <h4 className="text-lg md:text-xl font-bold mb-1 md:mb-2">{item.title}</h4>
                               <p className="text-steel-400 text-sm md:text-base font-medium leading-relaxed">{item.desc}</p>
                            </div>
                         </div>
                       ))}
                    </div>
                 ...
                 <div className="relative">
                    <div className="bg-steel-900 rounded-[32px] md:rounded-[48px] p-8 md:p-12 border border-white/5 shadow-2xl">
                       <Award className="text-steel-500 mb-6 md:mb-8" size={48} />
                       <h3 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6">Zero-Defect Policy</h3>
                       <p className="text-steel-400 text-sm md:text-base font-medium mb-8 md:mb-10 leading-relaxed italic">
                        ...
                        <Link to="/about" className="w-full md:w-auto text-center px-10 py-5 bg-primary text-white font-bold rounded-2xl inline-block hover:scale-105 transition-transform shadow-lg">
                          Read More About Us
                       </Link>
                    </div>
        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {posts.map((post, i) => (
              ...
              >
                <div className="aspect-[16/10] overflow-hidden rounded-[40px] mb-8 shadow-xl">
