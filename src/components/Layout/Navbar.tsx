          ...
          <button
            className={cn(
              ...
              isScrolled 
                ...
                : "bg-white text-steel-950 border border-white"
            )}
          ...
          >
            <div className="p-3 border-b border-steel-100 flex justify-between items-center bg-white sticky top-0 z-10">
              ...
              <button 
                ...
                className="p-2 text-steel-400 hover:text-steel-950 transition-colors bg-steel-50 rounded-xl"
              >
                <X size={20} />
              </button>
            </div>
            
            <div className="flex-grow flex flex-col p-5 space-y-1 overflow-y-auto bg-white">
              {navLinks.map((link) => (
                  ...
                  {link.hasDropdown ? (
                    <>
                      <button
                        ...
                        className={cn(
                          "text-sm font-bold py-4 px-2 flex justify-between items-center border-b border-steel-50 transition-colors w-full",
                      ...
                      <AnimatePresence>
                        {isServicesOpen && (
                          <motion.div
                            ...
                            className="bg-steel-50/50 rounded-xl overflow-hidden"
                          >
                            <div className="p-4 space-y-1">
                              {SERVICES.map(s => (
                                <Link 
                                  ...
                                  to={`/services?id=${s.id}`}
                                  onClick={() => setIsOpen(false)}
                                  className="block py-3 px-2 text-[10px] font-bold text-steel-600 uppercase tracking-widest border-b border-steel-100 last:border-none hover:text-primary"
                                >
                                  • {s.title}
                                </Link>
                              ))}
                    ...
                    <Link
                      ...
                      onClick={() => setIsOpen(false)}
                      className={cn(
                        "text-sm font-bold py-4 px-2 flex justify-between items-center border-b border-steel-50 transition-colors",
                        location.pathname === link.path ? "text-primary bg-brand-red-50/50 rounded-xl border-none" : "text-steel-950"
                      )}
                    >
                      {link.name}
            ...
            <div className="p-5 border-t border-steel-100 space-y-4 bg-white">
              <Link 
                to="/contact" 
                onClick={() => setIsOpen(false)}
                className="w-full py-4 bg-primary text-white text-[10px] font-bold uppercase tracking-widest rounded-xl text-center flex items-center justify-center gap-2 shadow-xl shadow-primary/20"
              >
